...
document.addEventListener('DOMContentLoaded', function () {
    const runButton = document.querySelector('#runScript');
    // onClick's logic below:
    runButton.addEventListener('click', (event) => {
        event.preventDefault;
        API_INPUT = document.querySelector('#api-key')
        if (API_INPUT.value) {
            const TEST_API = `${TEST_URL}&part=contentDetails&key=${API_INPUT.value}`
...
function verifyAPI_KEY(API_KEY) {
    console.log(API_KEY);
}
...
function runScript(API_KEY) {
    ...
    function getVideoId(bookmarks) {
        bookmarks.forEach(function (bookmark) {
            if (bookmark.url.startsWith(`${TEST_URL}`)) { // insert TEST_URL vs (YT_PREFIX) here to reduce API calls for testing
